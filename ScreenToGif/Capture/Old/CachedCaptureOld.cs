...
internal class CachedCaptureOld : ScreenCaptureOld
{
    #region Variables
    ...
    public override void Start(int delay, int left, int top, int width, int height, double scale, ProjectInfo project)
    {
        ...
        PixelOperations = CopyPixelOperations.SourceCopy;
        ...
        if (!SystemParameters.IsRemoteSession || !UserSettings.All.RemoteImprovement)
            PixelOperations |= CopyPixelOperations.CaptureBlt;

        //Bitmap details for each frame being captured.
        _bitmapHeader = new BitmapInfoHeader();
        _bitmapHeader.Size = (uint)Marshal.SizeOf(_bitmapHeader);
        _bitmapHeader.BitCount = 24; //Without alpha channel.
        _bitmapHeader.ClrUsed = 0;
        _bitmapHeader.ClrImportant = 0;
        _bitmapHeader.Compression = 0;
        _bitmapHeader.Height = -StartHeight; //Negative, so the Y-axis will be positioned correctly.
        _bitmapHeader.Width = StartWidth;
        _bitmapHeader.Planes = 1;
    ...
    public override int Capture(FrameInfo frame)
    {
        try
        {
            if (!Gdi32.StretchBlt(CompatibleDeviceContext, 0, 0, StartWidth, StartHeight, WindowDeviceContext, Left, Top, Width, Height, PixelOperations))
                return FrameCount;
            ...
            if (Gdi32.GetDIBits(WindowDeviceContext, CompatibleBitmap, 0, (uint)StartHeight, frame.Data, ref _bitmapHeader, DibColorModes.RgbColors) == 0)
    ...
    private void CaptureCursor()
    {
        try
        {
            ...
            {
                //Create bitmap.
                var compatibleBitmap = Gdi32.CreateCompatibleBitmap(WindowDeviceContext, colorHeader.Width, colorHeader.Height);
                var oldBitmap = Gdi32.SelectObject(CompatibleDeviceContext, compatibleBitmap);

                //Draw image.
                var ok = User32.DrawIconEx(CompatibleDeviceContext, 0, 0, cursorInfo.CursorHandle, 0, 0, CursorStep, IntPtr.Zero, DrawIconFlags.Image);

                if (!ok)
                {
                    CursorStep = 0;
                    User32.DrawIconEx(CompatibleDeviceContext, 0, 0, cursorInfo.CursorHandle, 0, 0, CursorStep, IntPtr.Zero, DrawIconFlags.Normal);
                }
                else
                    CursorStep++;

                colorHeader.Height *= -1;
                colorBuffer = new byte[colorHeader.SizeImage];

                Gdi32.GetDIBits(WindowDeviceContext, compatibleBitmap, 0, (uint)(colorHeader.Height * -1), colorBuffer, ref colorHeader, DibColorModes.RgbColors);

                //Erase bitmaps.
                Gdi32.SelectObject(CompatibleDeviceContext, oldBitmap);
                Gdi32.DeleteObject(compatibleBitmap);
            }
            ...
            {
                maskBuffer = new byte[maskHeader.SizeImage];
    ...
    private void FallbackCursorCapture2(FrameInfo frame)
    {
        try
        {
            ...
            if (colorHeader.Height != 0)
            {
                ...
                //Temporary.
                var img = ImageHelper.FromArray(colorBuffer, colorHeader.Width, colorHeader.Height * -1, channels, colorHeader.BitCount);
            ...
            if (maskHeader.Height != 0)
            {
                ...
                //Temporary.
                var image = ImageHelper.FromArray(maskBuffer, maskHeader.Width, maskHeader.Height * -1, 1, maskHeader.BitCount);
    ...
    private void FallbackCursorCapture3(FrameInfo frame)
    {
        ...
        try
        {
            ...
            if (User32.GetIconInfo(hicon, out var iconInfo))
            {
                ...
                {
                    var img = ImageHelper.FromArray(cursorShapeBuffer.ToList(), bitmapColor.bmWidth, bitmapColor.bmHeight, 4);
                ...
                var img2 = ImageHelper.FromArray(cursorMask.ToList(), bitmapMask.bmWidth, bitmapMask.bmHeight, 1);
